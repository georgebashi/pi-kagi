...
The `summarize` tool SHALL accept a `url` parameter (string, required), a `summary_type` parameter (enum: "summary" | "takeaway", optional, default "summary"), and a `target_language` parameter (string, optional). The tool SHALL send a request to the Kagi Universal Summarizer API at `POST https://kagi.com/api/v0/summarize` and return the summary text.
